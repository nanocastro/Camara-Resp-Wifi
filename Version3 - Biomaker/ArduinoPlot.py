...
* Descripción: Este programa lee datos del puerto serie utilizando un hilo que corre en background y envía los
*  datos al hilo principal.
*
* Change Log:
* -
'''
...
class myWindow(QtWidgets.QWidget):
    ...
    def __init__(self, *argv, **kwargv):
        ...
        self.timeStamp = deque([], maxlen=myWindow.MAX_DATOS)
        self.linRegress = []
    ...
    def updatePlot(self):
        ...
        if len(self.linRegress) > 0:
            ...
            # Calculamos la ordenada al tiempo inicial de la regresión
            co2Initialppm = self.linRegress.slope * self.minRange.value() + self.linRegress.intercept
    ...
    def exportarCSV(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self,"Guardar CSV","","All Files (*);;CSV Files (*.csv)", options=options)
        if not fileName.endswith('.csv'):
            fileName += '.csv'
        dataFrame = pd.DataFrame({"TimeStamp": self.timeStamp,
                                  "Tiempo": self.tiempo,
                                  "Medición": self.amplitud})
        dataFrame.to_csv(fileName, index=False)
        #np.savetxt(fileName,
        #           np.column_stack((self.timeStamp, self.tiempo, self.amplitud)),
        #           fmt='%.1f', delimiter=',', header="tiempo,amplitud")

    def exportFig(self):
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getSaveFileName(self,"Guardar Plot","","All Files (*);;PNG Files (*.png)", options=options)
        if not fileName.endswith('.png'):
            fileName += '.png'

        self.plotCanvas.fig.savefig(fileName, dpi=150)
